...
Ao todo serão feitos 4 jogos. Cada jogo terá um, ou mais, ganhadores. O jogador que obtiver mais pontos ao final dos 4 jogos será o vecedor. 
...
placar1 %>% 
    rename("Leiloeiro" = leiloeiro, "Quantidade de processos" = qtd_processos, "Score jogo 1" = score_jogo_1) %>% 
...
left_join(placar1, placar2, by = "leiloeiro") %>% 
  mutate(pontos_acumulados = score_jogo_1 + score_jogo_2) %>% 
  select(leiloeiro, vtp, score_jogo_2, pontos_acumulados) %>% 
  arrange(desc(score_jogo_2)) %>%
  rename("Leiloeiro" = leiloeiro, "Valor médio por processo" = vtp, "Score jogo 2" = score_jogo_2, "Pontos acumulados" = pontos_acumulados) %>%
  knitr::kable(digits=2)
```
...
left_join(placar3, left_join(placar1, placar2, by = "leiloeiro"), by = "leiloeiro") %>% 
  ...
  arrange(desc(score_jogo_3)) %>% 
  rename("Leiloeiro" = leiloeiro, "Taxa de sucesso" = taxa_sucesso, "Score jogo 3" = score_jogo_3, "Pontos acumulados" = pontos_acumulados) %>%
  knitr::kable(digits=2)
```
...
media4 <- mean(sucesso$taxa_venda)
...
left_join(placar4, left_join(placar3, left_join(placar1, placar2, by = "leiloeiro"), by = "leiloeiro"), by = "leiloeiro") %>% 
  ...
  arrange(desc(score_jogo_4)) %>%
  rename("Leiloeiro" = leiloeiro, "Taxa de venda média" = taxa_venda_media, "Score jogo 4" = score_jogo_4, "Pontos acumulados" = pontos_acumulados) %>%
  knitr::kable(digits=2)
```
...
left_join(placar4, left_join(placar3, left_join(placar1, placar2, by = "leiloeiro"), by = "leiloeiro"), by = "leiloeiro") %>% 
  ...
  rename("Leiloeiro" = leiloeiro, "Score jogo 1" = score_jogo_1, "Score jogo 2" = score_jogo_2, "Score jogo 3" = score_jogo_3, "Score jogo 4" = score_jogo_4, "Pontos acumulados" = pontos_acumulados, "Colocação final" = score_total) %>%
  knitr::kable(digits=2) %>% 
...
A primeira ressalva diz respeito ao que significa ser ganhador desta competição. O ponto que quero deixar claro é que o sucesso de um leiloeiro não necessariamente está ligado aos seus próprios esforços. Pela narrativa da "competição" que eu criei para apresentar os dados, poderia dar a entender que os leiloeiros ganhadores são aqueles que se esforçaram mais para ganhar e, portanto, são os melhores leiloeiros. Entretanto, o que eu preciso deixar claro é que o sucesso dos leiloeiros em cada um dos jogos envolvia fatores fatores que estavam muito além das vontades e capacidades de cada jogador. Vejamos caso a caso onde estavam as limitações. 
...
A segunda ressalva diz respeito a alguns valores em específico sobre os quais devemos ter cautela. Refiro-me aos valores dos **jogos 3 e 4**. O que ocorre nestes jogos é que
