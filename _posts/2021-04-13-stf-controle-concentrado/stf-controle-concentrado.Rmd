...
preview: "grafico_perfil_ato_federal_questionado-1.png"
---
...
base <- readr::read_rds("base_referencia.rds")
base_partes <- readr::read_rds("base_partes.rds")
base_partes_cat <- readr::read_rds("base_partes_cat.rds")
base_temas <- readr::read_rds("base_temas.rds")

base_partes_categorizada_tipo_ato <- base_partes_cat %>%
...
```{r grafico_perfil_ato_federal_questionado, echo=FALSE, message=FALSE, warning=FALSE}

base_partes_categorizada_tipo_ato %>%
...
```{r grafico_esfera_ato_questionado, echo=FALSE, message=FALSE, warning=FALSE}
base_partes_categorizada_tipo_ato %>%
...
#        subtitle = "Agrupado pelas categorias de proponentes",
#        caption = "Até 31/12/2020")
knitr::include_graphics("grafico_quem_covid-1.png")
```
...
base_tema_pandemia <- base_partes_cat %>%
  filter(ano == 2020) %>%
...
base_textos_pandemia <- base_tema_pandemia %>%
  left_join(readr::read_rds("palavra_chave_nest.rds")) %>%
...
base_textos_nuvem_categoria <- readr::read_rds("palavra_chave_nest.rds") %>%
  tidyr::unnest() %>%
  tidyr::unnest() %>%
  left_join(base_partes_cat %>%
