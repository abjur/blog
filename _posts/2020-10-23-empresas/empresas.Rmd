...
Para lidar com esse problema, utilizamos a classificação dos processos por tipo. A classificação foi contruída a partir dos temas e condutas, que foram discutidos em outro artigo.
...
A lista de empresas envolvidas nos PAS foi obtida através da busca dos CNPJs nos autos ou da busca das empresas envolvidas em sites que agregam dados do site da Receita Federal do Brasil (RFB), como o [cnpj.info](https://cnpj.info). No total, foram encontrados `r n_cnpj` distintos. 
...
A base final dos `r nrow(da_tipo_empresa)` CNPJs ficou com a seguinte classificação da Figura \@ref(fig:tipo-empresa-final). É possível notar que uma parte considerável dos casos foi classificada como não relacionada ao caso, mas que isso foi feito de forma aproximada em somente de 10% dos casos. Quase um quarto dos CNPJs estão fora da base por problemas de registro. Dentre os CNPJs encontrados, o mais comum é o de administradores de carteira, que aparecem em aproximadamente um quinto dos casos.
...
da_tipo_empresa %>% 
  dplyr::mutate(classif = dplyr::case_when(
    tipo == "Não relacionada*" ~ "Não encontrado",
    TRUE ~ "Normal"
  )) %>% 
  dplyr::mutate(tipo = dplyr::case_when(
