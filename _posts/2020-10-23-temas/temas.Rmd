...
```{r condutas}
obsMC::da_tema_conduta %>%
  dplyr::filter(!is.na(conduta)) %>%
  dplyr::mutate(
    conduta = stringr::str_replace(
      conduta,
      "Autorização da CVM para administação de carteiras",
      "Registro na CVM para administação de carteiras"
    ),
    dispositivo_art = stringr::str_replace(dispositivo_art, "ICVM 306/1999, 3", "Lei 6385, 23")
  ) %>%
  dplyr::group_by(tema, conduta) %>%
  dplyr::mutate(tema = dplyr::case_when(
    tema =  stringr::str_detect(
      conduta,
      stringr::regex("Normas de conduta de administradores de carteiras")
    ) ~ "Ilícitos de mercado",
    TRUE ~ tema
  )) %>%
  dplyr::count(conduta, tema, motivo, dispositivo_art) %>% 
  # dplyr::summarise(
  ...
  #   .groups = "drop"
  # ) %>%
  dplyr::mutate(prop = n / dplyr::n_distinct(obsMC::da_tema_conduta$n_interno_abj)) %>%
