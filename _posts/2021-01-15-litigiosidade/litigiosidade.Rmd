...
base_litigiosidade <- inner_join(base1, base2, by = "codigo_ibge") %>%
  ...
  mutate(taxa = round(((qtd_processos/pop)*10000), 3)) 
...
```
### 5 municípios (comarcas) com maiores taxas de litigiosidade (Por região)

```{r,  eval = TRUE, echo = FALSE}
base_litigiosidade %>% 
  group_by(taxa) %>% 
  dplyr::count(regiao_nm, taxa)  
...
```{r litigiosidade-idhm_e, fig.cap = ".", layout="l-body-outset", eval = TRUE, echo = FALSE}
base_litigiosidade %>%
  dplyr::select(idhm_e, taxa, regiao_nm) %>%
  dplyr::mutate(media_taxa = mean(base_litigiosidade$taxa - base_litigiosidade$idhm_e)) %>%
  ggplot2::ggplot() +
  #ggplot2::geom_hline(yintercept = media_taxa, size = .5, colour = "red", linetype = 2) +
  ggplot2::geom_point(aes(x = idhm_e,  y = taxa), color = "#102C68")  +
...
```{r litigiosidade-gini, fig.cap = ".", layout="l-body-outset", eval = TRUE, echo = FALSE}
base_litigiosidade %>%
  dplyr::select(taxa, gini, regiao_nm) %>%
  ggplot2::ggplot() +
  ggplot2::geom_col(aes(x = gini, y = taxa), fill = viridisLite::viridis(1, 1, .2, .8))  +
