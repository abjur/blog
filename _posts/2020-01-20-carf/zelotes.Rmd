...
```{r}
d_vis <- readRDS("../Downloads/d_vis.rds")
...
## Volume de decisões
Em 2015 e 2016, logo após o início da Operação Zelotes, a quantidade de decisões publicadas pelo Carf foi inferior ao dos outros anos analisados. Vale lembrar também que, além das perturbações causadas pelas investigações, em junho de 2015 o Carf teve seu novo regimento interno aprovado -- cujo texto sofreu alterações ao longo de 2016, 2017 e 2018.

```{r}
# Volume de decisões
d_vis %>% 
  group_by(ano, ano_mes, zelotes) %>% summarise(n = n()) %>%
  arrange(ano_mes) %>%
  ggplot() +
  # geom_col(aes(x = ano_mes, y = n, fill = ano)) +
  geom_col(aes(x = ano_mes, y = n, fill = zelotes)) +
  labs(y = element_blank(), x = element_blank(), title = "Quantidade de decisões publicadas por mês") +
...
A quantidade de resultados favoráveis ao contribuinte superou o total de resultados desfavoráveis a partir de 2016, chegando a ser o dobro em alguns anos. É interessante notar que esse novo padrão se manteve ao longo do tempo, não se contendo a apenas o período inicial da operação.
...
d_vis %>% 
  ...
  geom_line() +
  scale_color_manual(values = c(wes_palette(n = 6, name = "IsleofDogs1"), wes_palette(n = 3, name = "IsleofDogs2"))) +
