...
aux_plot <- aux_valor %>% 
  ...
  # dplyr::filter(n > 25) %>% 
  dplyr::mutate(dplyr::across(-c(tempo, n), ~./max(., na.rm = TRUE)))

aux_plot %>% 
  purrr::set_names(c("Intervalo", "N", "Taxa (capital)",
                     "Taxa (valor pago)", "Taxa (faturamento)")) %>% 
  knitr::kable(
...
aux_plot %>%
  dplyr::filter(n > 30) %>% 
  tidyr::pivot_longer(c(capital, total, faturamento)) %>% 
