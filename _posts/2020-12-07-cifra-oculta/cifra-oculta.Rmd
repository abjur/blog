...
assuntos %>%
  ...
  count() %>%
  arrange(desc(n)) %>% 
  plyr::rename(c("assunto_nome1" = "Assunto", "n" = "Quantidade")) %>% 
...
```{r fig.cap = "Contagem de Assuntos de forma geral"}
assuntos %>%
  select(assunto_nome1, x1_grau) %>%
  group_by(assunto_nome1) %>%
  count() %>%
...
```{r fig.cap = "Contagem de casos por Ano"}
assuntos %>%
  group_by(ano) %>%
  count() %>%
  arrange(desc(n)) %>%
...
```{r fig.cap = "Contagem de casos por Tribunal"}
assuntos %>%
...
```{r fig.cap = "Taxa de cifra oculta por Assunto"}
assuntos %>%
  group_by(assunto_nome1) %>%
...
```{r fig.cap = "Taxa de cifra oculta por Tribunal"}
assuntos %>%
  group_by(tribunal) %>%
  summarise(taxa = sum(x1_grau[generico == "Sim"]) / sum(x1_grau), .groups = "drop") %>%
  ggplot() +
