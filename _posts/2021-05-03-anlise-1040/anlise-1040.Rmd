...
Para analisar os dados, foi necessário realizar alguns filtros de limpeza. Primeiro, retiramos casos que relataram o número de horas de forma aproximada, em múltiplos de 12 horas. Além disso, consideramos apenas casos com tempos de análise de viabilidade de nome maiores que zero.
...
da_redesim <- redesim::redesim_min %>% 
  dplyr::filter(
    ano_mes >= "2020-01-01",
    ano_mes <= "2021-03-01",
    qtde_hh_viabilidade_total %% 24 != 0,
...
A média geral é de `r pct(prop_viab_total)`, sendo que existe alta variabilidade entre os estados. Amazonas e Ceará apresentam proporções maiores de 50% de registros impactados, enquanto Rio de Janeiro, Pernambuco, Santa Catarina, São Paulo e Pará apresentam proporções menores de 5%.
...
A Figura \@ref(fig:prop-viab-end) mostra os resultados das simulações. Os tamanhos das barras são as estimativas de redução no tempo total de registro na Unidade Federativa considerada, e as cores das barras indicam os três cenários considerados. É possível notar que as Unidades Federativas com maior potencial de impacto da MPV são Amazonas e Paraíba. Já Pernambuco, Rio de Janeiro, Santa Catarina, São Paulo e Pará provavelmente serão pouco afetadas.

```{r prop-viab-end, fig.cap = "Estimativas de redução no tempo total de registro, considerando mil simulações da base de dados da Redesim e cenários com 20%, 50% e 80% de adoção da MPV.", fig.width=8, fig.height=8, layout="l-body-outset", dpi=500}
...
No cenário pessimista, a redução esperada é de `r pct(totais$mu_total[3])`, variando entre zero e 4% nas Unidades Federativas. Já no cenário moderado, a redução esperada é de `r pct(totais$mu_total[2])`, variando entre zero quase 10%. No cenário otimista, a redução esperada é de `r pct(totais$mu_total[1])`, variando entre zero e pouco mais de 15%.

Para finalizar, é importante reiterar que a presente análise não considera fatores externos, como tendência geral de queda no tempo dos registros e impactos da pandemia de Covid-19, entre outros. Trata-se de um exercício de simulação, a ser utilizado mais como um guia do que como uma predição.
